        ...
        <div class="row">
            @foreach ($posts->skip(1) as $post)
                <div class="col-md-4 mb-3">
                    <div class="card">
                        <div class="position-absolute bg-dark text-light px-3 py-2" style="background-color: rgba(0, 0, 0, 0.7)"><a href="/posts?category={{ $post->category->slug }}" class="text-light text-decoration-none">{{ $post->category->name }}</a></div>
                        @if ($post->image)
                        ...
                        <a href="/posts/{{ $post->slug }}" class="btn btn-info text-light">Read More</a>
                        </div>
                    </div>
                </div>
            @endforeach
